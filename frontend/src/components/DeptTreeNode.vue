    ...
    <div :style="{ paddingLeft: (depth * 2) + 'rem' }" 
      ...
      <div class="flex-1 min-w-0">
        <div class="flex items-center gap-2">
          ...
          <span v-if="node.children && node.children.length > 0" class="text-[0.75rem] font-black text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-lg">
            {{ node.children.length }}
          </span>
        </div>
        <div class="text-[0.75rem] text-slate-400 font-bold uppercase tracking-widest mt-0.5">#{{ node.id }}</div>
      </div>
