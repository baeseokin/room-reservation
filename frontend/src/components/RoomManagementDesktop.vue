      ...
      <div v-else class="divide-y divide-slate-100">
        <div v-for="room in filteredRooms" :key="room.id"
          ...
          <div class="w-full md:w-40 h-28 shrink-0 rounded-2xl overflow-hidden border border-slate-100 bg-slate-50 relative group">
            ...
            <div class="absolute top-2 left-2 px-2 py-1 bg-slate-900/80 backdrop-blur text-white text-[0.75rem] font-black rounded-lg uppercase tracking-tighter">
              {{ room.floor }}층
            </div>
          </div>
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-3 mb-1">
              ...
              <span class="text-[0.75rem] font-black text-indigo-600 bg-indigo-50 px-2.5 py-1 rounded-full uppercase tracking-widest">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-y-2 gap-x-6 mt-3">
              <div class="flex items-center gap-2">
                <span class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest w-12">담당자</span>
                <span class="text-xs font-bold text-slate-700">{{ room.manager_name || '미지정' }}</span>
              </div>
              <div class="flex items-center gap-2">
                <span class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest w-12">연락처</span>
                <span class="text-xs font-bold text-slate-700">{{ formatPhone(room.manager_contact) }}</span>
              </div>
              <div class="flex items-center gap-2">
                <span class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest w-12">수용인원</span>
              ...
              <div v-if="room.blocked_times?.length > 0" class="flex flex-wrap gap-1 col-span-full mt-2">
                <span class="text-[0.75rem] font-black text-rose-500 uppercase tracking-widest w-full mb-1">예약 불가 시간</span>
                <div v-for="bt in room.blocked_times" :key="'list-bt-'+bt.id" 
                  ...
                  <span class="text-[0.75rem] font-black text-rose-600">{{ getDayLabel(bt.day_of_week) }}</span>
                  <span class="text-[0.75rem] font-bold text-slate-600">{{ bt.start_time.slice(0,5) }}-{{ bt.end_time.slice(0,5) }}</span>
                </div>
              </div>
            </div>
          </div>
      ...
      <div v-if="showModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-xl flex items-center justify-center p-4 z-[100]">
        <div class="bg-white rounded-[3rem] shadow-2xl max-w-lg w-full p-10 space-y-8 animate-in fade-in zoom-in duration-300 overflow-y-auto max-h-[90vh] scrollbar-hide">
          <div class="flex justify-between items-start">
            <div class="space-y-1">
              <span class="text-[0.75rem] font-black text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full uppercase tracking-widest">시설 정보 설정</span>
          ...
          <form @submit.prevent="saveRoom" class="space-y-6">
            <div class="space-y-4">
              <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                  <label class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest ml-1">공간명</label>
                ...
                <div class="space-y-1.5">
                  <label class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest ml-1">수용인원 (명)</label>
              ...
              <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                  <label class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest ml-1">위치 (층)</label>
                ...
                <div class="space-y-1.5">
                  <label class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest ml-1">소속 부서</label>
            ...
            <div class="space-y-4 pt-4 border-t border-slate-100">
              <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                  <label class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest ml-1">담당자</label>
                ...
                <div class="space-y-1.5">
                  <label class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest ml-1">연락처</label>
              ...
              <div class="space-y-2">
                <label class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest ml-1">공간 사진</label>
    ...
    <Teleport to="body">
      <div v-if="showBlockedModal" class="fixed inset-0 bg-slate-900/60 backdrop-blur-xl flex items-center justify-center p-4 z-[100]">
        <div class="bg-white rounded-[3rem] shadow-2xl max-w-2xl w-full p-10 space-y-8 animate-in fade-in zoom-in duration-300 overflow-y-auto max-h-[90vh] scrollbar-hide">
          <div class="flex justify-between items-start">
            <div class="space-y-1">
              <span class="text-[0.75rem] font-black text-rose-600 bg-rose-50 px-3 py-1 rounded-full uppercase tracking-widest">예약 불가 설정</span>
          ...
          <div class="grid grid-cols-1 md:grid-cols-1 gap-8">
            ...
            <div class="bg-slate-50 rounded-3xl p-8 space-y-6">
              ...
              <div class="space-y-4">
                ...
                <div class="space-y-1.5">
                  <label class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest ml-1">반복 유형</label>
                  <div class="flex gap-2">
                    <button v-for="t in [['weekly','주 단위'], ['monthly_date','월 단위 (일자)'], ['monthly_nth','월 단위 (요일)']]" :key="t[0]"
                            ...
                            class="flex-1 py-3 rounded-2xl text-[0.75rem] font-black uppercase tracking-widest transition-all">
                      {{ t[1] }}
                    </button>
                  </div>
                </div>
                ...
                <div v-if="blockedForm.recurring_type === 'weekly'" class="space-y-1.5">
                  <label class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest ml-1">요일</label>
                ...
                <div v-else-if="blockedForm.recurring_type === 'monthly_date'" class="space-y-1.5">
                  <label class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest ml-1">매월 일자</label>
                ...
                <div v-else-if="blockedForm.recurring_type === 'monthly_nth'" class="grid grid-cols-2 gap-4">
                  <div class="space-y-1.5">
                    <label class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest ml-1">주차</label>
                ...
                <div class="grid grid-cols-2 gap-4">
                  <div class="space-y-1.5">
                    <label class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest ml-1">시작 시간</label>
                    <div class="flex gap-2">
                  ...
                  <div class="space-y-1.5">
                    <label class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest ml-1">종료 시간</label>
                    <div class="flex gap-2">
                ...
                <!-- Row 2: Reason -->
                <div class="space-y-1.5">
                  <label class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest ml-1">차단 사유</label>
              ...
              <div v-else class="space-y-2">
                <div v-for="bt in blockedTimes" :key="bt.id" class="flex items-center justify-between p-4 bg-white border border-slate-100 rounded-2xl group hover:border-rose-200 transition-all">
                  <div class="flex items-center gap-4">
                    <div class="bg-rose-50 text-rose-600 text-[0.75rem] font-black px-2.5 py-1 rounded-lg uppercase tracking-widest whitespace-nowrap">
                    ...
                    <div>
                      ...
                      <div class="text-[0.75rem] font-bold text-slate-400 uppercase tracking-widest">{{ bt.reason || '사유 없음' }}</div>
                    </div>
                  </div>
