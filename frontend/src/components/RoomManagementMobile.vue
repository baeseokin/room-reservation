...
const openModal = (room = null) => {
  ...
  if (room) {
    ...
    form.value = { room_name: '', capacity: '', floor: '', dept_name: departments.value[0]?.dept_name || '', manager_name: '', manager_contact: '', image_url: null }
  }
      ...
      <div v-for="room in rooms" :key="room.id" class="bg-white p-5 rounded-3xl shadow-sm border border-slate-100 flex items-center justify-between">
        <div class="flex items-center gap-4">
           ...
           <div>
             ...
             <p class="text-[0.75rem] text-slate-400">{{ room.dept_name }} | {{ room.manager_name }} | {{ formatPhone(room.manager_contact) }}</p>
             <p class="text-[0.75rem] text-indigo-500 font-bold">수용인원: {{ room.capacity || '-' }}명</p>
             ...
             <div v-if="room.blocked_times?.length > 0" class="flex flex-wrap gap-1 mt-2">
               <div v-for="bt in room.blocked_times" :key="'mob-bt-'+bt.id" 
                    class="px-1.5 py-0.5 bg-rose-50 text-[0.75rem] font-black text-rose-600 rounded">
          ...
          <div class="space-y-4">
            <div class="bg-slate-50 p-6 rounded-[2rem] space-y-4">
              ...
              <div class="flex gap-1.5 p-1 bg-white rounded-2xl shadow-sm">
                <button v-for="t in [['weekly','주간'], ['monthly_date','월간(일)'], ['monthly_nth','월간(요일)']]" :key="t[0]"
                        ...
                        class="flex-1 py-2.5 rounded-xl text-[0.75rem] font-black uppercase tracking-widest transition-all">
                  {{ t[1] }}
                </button>
              </div>
              ...
              <div v-if="blockedForm.recurring_type === 'weekly'" class="flex items-center gap-3">
                <span class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest w-10">요일</span>
              ...
              <div v-if="blockedForm.recurring_type === 'monthly_date'" class="flex items-center gap-3">
                <span class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest w-10">일자</span>
              ...
              <div v-if="blockedForm.recurring_type === 'monthly_nth'" class="space-y-3">
                <div class="flex items-center gap-3">
                  <span class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest w-10">주차</span>
              ...
              <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                  <span class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest ml-1">시작 시간</span>
                  <div class="flex gap-1.5">
                    <select v-model="startHour" class="flex-1 bg-white border-none rounded-xl py-3 px-1 text-[0.6875rem] font-bold appearance-none text-center shadow-sm">
                      <option v-for="h in hours.slice(0, 24)" :key="'msh-'+h" :value="h">{{ h }}시</option>
                    </select>
                    <select v-model="startMin" class="flex-1 bg-white border-none rounded-xl py-3 px-1 text-[0.6875rem] font-bold appearance-none text-center shadow-sm">
                      <option v-for="m in minutes" :key="'msm-'+m" :value="m">{{ m }}분</option>
                    </select>
                  </div>
                </div>
                <div class="space-y-1.5">
                  <span class="text-[0.75rem] font-black text-slate-400 uppercase tracking-widest ml-1">종료 시간</span>
                  <div class="flex gap-1.5">
                    <select v-model="endHour" class="flex-1 bg-white border-none rounded-xl py-3 px-1 text-[0.6875rem] font-bold appearance-none text-center shadow-sm">
                      <option v-for="h in hours" :key="'meh-'+h" :value="h">{{ h }}시</option>
                    </select>
                    <select v-model="endMin" class="flex-1 bg-white border-none rounded-xl py-3 px-1 text-[0.6875rem] font-bold appearance-none text-center shadow-sm">
            ...
            <div class="space-y-2">
              <div v-for="bt in blockedTimes" :key="bt.id" class="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
                <div class="flex items-center gap-3">
                  <span class="bg-rose-100 text-rose-600 text-[0.75rem] font-black px-2 py-1 rounded-lg">
