  ...
  <div class="space-y-6 pb-20">
    ...
    <div class="flex justify-between items-center px-2">
      <div class="space-y-1">
        ...
        <p class="text-slate-400 text-[0.6875rem] font-black uppercase tracking-widest">시설물 등록 및 관리 설정을 관리합니다.</p>
      </div>
    ...
    <div class="flex gap-2 overflow-x-auto pb-2 px-2 scrollbar-hide">
      <button @click="filterFloor = 'all'"
        ...
        class="px-5 py-2.5 rounded-2xl text-[0.625rem] font-black uppercase tracking-widest transition-all shrink-0">
        전체
      </button>
    ...
    <div v-else-if="filteredRooms.length === 0" class="text-center py-32 bg-white rounded-[2.5rem] border border-slate-100 mx-2">
      ...
      <p class="text-slate-400 font-black text-[0.625rem] uppercase tracking-widest">등록된 공간이 없습니다</p>
    </div>
    ...
    <div v-else class="space-y-4 px-2">
      <div v-for="room in filteredRooms" :key="room.id"
        ...
        <div v-if="room.image_url" class="w-full h-48 relative">
          ...
          <div class="absolute top-4 right-4 bg-slate-900/80 backdrop-blur text-white px-3 py-1.5 rounded-full text-[0.625rem] font-black uppercase tracking-widest">
            {{ room.floor }}층
          </div>
        </div>
        ...
        <div class="p-6 space-y-6">
          ...
          <div class="flex justify-between items-start">
            <div class="space-y-1">
              <div class="flex items-center gap-2">
                <span v-if="!room.image_url" class="bg-slate-100 text-slate-500 px-2 py-1 rounded-lg text-[0.625rem] font-black uppercase tracking-widest">
                  {{ room.floor }}층
                </span>
                <span class="bg-indigo-50 text-indigo-600 px-2 py-1 rounded-lg text-[0.625rem] font-black uppercase tracking-widest">
          ...
          <div class="grid grid-cols-2 gap-4">
            <div class="bg-slate-50 p-4 rounded-3xl space-y-1">
              <span class="flex items-center gap-1.5 text-[0.5625rem] font-black text-slate-400 uppercase tracking-widest">
          ...
          <div v-if="room.manager_contact" class="flex items-center gap-3 px-2">
            ...
            <div class="flex-1 min-w-0">
              <p class="text-[0.625rem] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">담당 연락처</p>
          ...
          <div v-if="room.blocked_times?.length > 0" class="space-y-2 px-2 border-t border-slate-100 pt-4">
            <span class="text-[0.625rem] font-black text-rose-500 uppercase tracking-widest block mb-2">예약 불가 시간</span>
            <div class="flex flex-wrap gap-2">
              <div v-for="bt in room.blocked_times" :key="'list-bt-'+bt.id" 
                ...
                <span class="text-[0.625rem] font-black text-rose-600">{{ getDayLabel(bt.day_of_week) }}</span>
                <span class="text-[0.625rem] font-bold text-slate-500">{{ bt.start_time.slice(0,5) }}-{{ bt.end_time.slice(0,5) }}</span>
              </div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-3 gap-2 pt-2">
            <button @click="openBlockedModal(room)" class="flex flex-col items-center justify-center gap-1.5 py-4 bg-white border border-slate-100 rounded-3xl text-indigo-600 active:scale-95 transition-all">
              ...
              <span class="text-[0.5625rem] font-black uppercase tracking-widest">차단설정</span>
            </button>
            <button @click="openModal(room.floor, room)" class="flex flex-col items-center justify-center gap-1.5 py-4 bg-white border border-slate-100 rounded-3xl text-slate-900 active:scale-95 transition-all">
              ...
              <span class="text-[0.5625rem] font-black uppercase tracking-widest">수정</span>
            </button>
            <button @click="deleteRoom(room.id)" class="flex flex-col items-center justify-center gap-1.5 py-4 bg-rose-50 rounded-3xl text-rose-500 active:scale-95 transition-all">
              ...
              <span class="text-[0.5625rem] font-black uppercase tracking-widest">삭제</span>
            </button>
          </div>
        </div>
      </div>
    ...
    <Teleport to="body">
      ...
      <div v-if="showModal" class="fixed inset-0 bg-slate-900/80 backdrop-blur-xl flex items-end sm:items-center justify-center z-[100]">
        <div class="bg-white w-full max-w-lg rounded-t-[3rem] sm:rounded-[3.5rem] p-8 sm:p-12 space-y-8 animate-in slide-in-from-bottom duration-500 overflow-y-auto max-h-[90vh]">
          <div class="flex justify-between items-center">
            <div class="space-y-1">
              <span class="text-[0.625rem] font-black text-indigo-600 bg-indigo-50 px-3 py-1 rounded-full uppercase tracking-widest">공간 설정</span>
            ...
            <div class="bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100 space-y-6">
              <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                  <label class="text-[0.6875rem] font-black text-slate-400 uppercase tracking-widest ml-1">공간명</label>
                ...
                <div class="space-y-1.5">
                  <label class="text-[0.6875rem] font-black text-slate-400 uppercase tracking-widest ml-1">수용인원</label>
              ...
              <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                  <label class="text-[0.6875rem] font-black text-slate-400 uppercase tracking-widest ml-1">위치 (층)</label>
              ...
              <div class="grid grid-cols-2 gap-4">
                <div class="space-y-1.5">
                  <label class="text-[0.6875rem] font-black text-slate-400 uppercase tracking-widest ml-1">담당자</label>
                ...
                <div class="space-y-1.5">
                  <label class="text-[0.6875rem] font-black text-slate-400 uppercase tracking-widest ml-1">연락처</label>
              ...
              <!-- Room Image Upload -->
              <div class="space-y-2">
                <label class="text-[0.6875rem] font-black text-slate-400 uppercase tracking-widest ml-1">공간 사진</label>
                ...
                <label v-else class="flex flex-col items-center justify-center w-full aspect-video bg-white border-2 border-dashed border-slate-200 rounded-[2.5rem] hover:bg-indigo-50 hover:border-indigo-300 transition-all cursor-pointer">
                  ...
                  <span class="text-[0.625rem] font-black text-slate-400 uppercase tracking-widest">사진 업로드 (선택)</span>
      ...
      <div v-if="showBlockedModal" class="fixed inset-0 bg-slate-900/80 backdrop-blur-xl flex items-end sm:items-center justify-center z-[100]">
        <div class="bg-white w-full max-w-lg rounded-t-[3rem] sm:rounded-[3.5rem] p-8 sm:p-12 space-y-8 animate-in slide-in-from-bottom duration-500 overflow-y-auto max-h-[90vh] scrollbar-hide">
          <div class="flex justify-between items-center">
            <div class="space-y-1">
              <span class="text-[0.625rem] font-black text-rose-500 bg-rose-50 px-3 py-1 rounded-full uppercase tracking-widest">예약 불가 시간</span>
          ...
          <div class="bg-slate-50 p-6 rounded-[2.5rem] border border-slate-100 space-y-6">
             <div class="space-y-1.5">
               <label class="text-[0.6875rem] font-black text-slate-400 uppercase tracking-widest ml-1">반복 유형</label>
               <div class="flex gap-2">
                 <button v-for="t in [['weekly','주 단위'], ['monthly_date','일자'], ['monthly_nth','요일차']]" :key="t[0]"
                         ...
                         class="flex-1 py-3 rounded-2xl text-[0.625rem] font-black uppercase tracking-widest transition-all">
                   {{ t[1] }}
                 </button>
               </div>
             </div>
             ...
             <div v-if="blockedForm.recurring_type === 'weekly'" class="space-y-1.5">
               <label class="text-[0.6875rem] font-black text-slate-400 uppercase tracking-widest ml-1">요일 선택</label>
               <div class="grid grid-cols-7 gap-1">
                 <button v-for="d in days" :key="d.val" 
                   ...
                   class="py-3 rounded-xl text-[0.625rem] font-black uppercase tracking-widest transition-all">
                   {{ d.label }}
                 </button>
               </div>
             </div>
             ...
             <div v-else-if="blockedForm.recurring_type === 'monthly_date'" class="space-y-1.5">
               <label class="text-[0.6875rem] font-black text-slate-400 uppercase tracking-widest ml-1">매월 일자</label>
             ...
             <div v-else-if="blockedForm.recurring_type === 'monthly_nth'" class="grid grid-cols-2 gap-4">
               <div class="space-y-1.5">
                 <label class="text-[0.6875rem] font-black text-slate-400 uppercase tracking-widest ml-1">주차 선택</label>
             ...
             <div class="grid grid-cols-2 gap-4 border-t border-slate-100 pt-6">
                <div class="space-y-1.5">
                  <label class="text-[0.6875rem] font-black text-slate-400 uppercase tracking-widest ml-1">시작</label>
                  <div class="flex gap-2">
                ...
                <div class="space-y-1.5">
                  <label class="text-[0.6875rem] font-black text-slate-400 uppercase tracking-widest ml-1">종료</label>
                  <div class="flex gap-2">
             ...
             <div class="space-y-1.5">
               <label class="text-[0.6875rem] font-black text-slate-400 uppercase tracking-widest ml-1">차단 사유</label>
          ...
          <div class="space-y-4">
            ...
            <div v-if="blockedTimes.length === 0" class="text-center py-10 bg-slate-50 rounded-[2rem] border border-dashed border-slate-200">
               <p class="text-[0.625rem] font-black text-slate-300 uppercase tracking-widest">등록된 차단 내역 없음</p>
            </div>
            <div v-else class="space-y-3">
              <div v-for="bt in blockedTimes" :key="bt.id" class="bg-white border border-slate-100 p-5 rounded-[2rem] flex items-center justify-between shadow-sm">
                <div class="space-y-1">
                  <div class="flex items-center gap-2">
                    <span class="text-[0.625rem] font-black text-rose-600 uppercase tracking-widest">
                  ...
                  <p class="text-[0.625rem] font-bold text-slate-400 uppercase tracking-widest">{{ bt.reason || '사유 미입력' }}</p>
                </div>
